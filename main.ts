...
function create_level_one() {
    ...
    `)
    music.stopAllSounds()
    music.play(music.createSong(assets.song`dungeonFloorsSong`), music.PlaybackMode.LoopingInBackground)
...
function create_level_three() {
    ...
    ball_found = true
    music.stopAllSounds()
    music.play(music.createSong(assets.song`arenaSong`), music.PlaybackMode.LoopingInBackground)
    scene.setTileMapLevel(tilemap`
        level three no exit
    `)
